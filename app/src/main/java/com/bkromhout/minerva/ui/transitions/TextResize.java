...
/**
 * Transitions a TextView from one font size to another. This does not do any animation of TextView content and if the
 * text changes, this transition will not run.
 * <p>
 * The animation works by capturing a bitmap of the text at the start and end states. It then scales the start bitmap
 * until it reaches a threshold and switches to the scaled end bitmap for the remainder of the animation. This keeps the
 * jump in bitmaps in the middle of the animation, where it is less noticeable than at the beginning or end of the
 * animation. This transition does not work well with cropped text. TextResize also does not work with changes in
 * TextView gravity.
 * <p>
 * Taken from Google's <a href="https://github.com/googlesamples/android-unsplash">android-unsplash</a> sample app
 * repo.
 */
public class TextResize extends Transition {
    ...
    /**
     * This Drawable is used to scale the start and end bitmaps and switch between them at the appropriate progress.
     */
    ...
    /**
     * Contains all the non-font-size data used by the TextResize transition. None of these values should trigger the
     * transition, so they are not listed in PROPERTIES. These are captured together to avoid boxing of all the
